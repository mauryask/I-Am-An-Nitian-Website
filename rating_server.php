...
/*<<<<<<<<===========================================>>>>>>>>>*/
  ...
  else if($action == 'dislike')   //onclik dislike 
  {
    ...
    if(mysqli_num_rows($result_x)>0)  // if user already exists
    {
     ...
     if($like_st == 0 && $dislike_st == 0)  //case 1
     {
       ...
       $decounter = 0;
       //updating state of like and dislike in rating_info table
      $sql_x = "update rating_info set dislike_st = {$counter}  where post_id= '".$post_id."'";   
  
      //updating number of likes and dislikes in rating_info table
      $sql_y = "update tbl_images set dislikes = dislikes + {$counter} where id= '".$post_id."'";
  ...
  else if($like_st == 0 && $dislike_st == 1)  //case 2
  {
   $counter = 1;
    $decounter=0;
    //updating state of like and dislike in rating_info table
   $sql_x = "update rating_info set  dislike_st = {$decounter} where post_id= '".$post_id."'";   
 ...
 else if($like_st == 1 && $dislike_st == 0)  //case 3
  {
   ...
   $decounter=0;
   //updating state of like and dislike in rating_info table
  $sql_x = "update rating_info set like_st = {$decounter} , dislike_st = {$counter}  where post_id= '".$post_id."'";   
  ...
  mysqli_query($conn, $sql_y);
    ...
    else // when user first time dislikes the post
    {
      $like_st = 0;
      $dislike_st = 1;
      $counter = 1;
